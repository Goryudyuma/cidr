...
状態をJSON → gzip → base64urlに変換し、URLの`#s=1.`以降に格納します。このフラグメントはHTTPリクエストに含まれず、計算API・DB・共有用サーバーは不要です。リンクの作成も読み込み後はオフラインで動きます。リンクを知っている人は入力途中の内容も復元できるため、共有前に内容を確認してください。暗号化やアクセス制限はありません。

共有リンクのフラグメントは32,768文字、圧縮前・展開後のJSONは1MiBまでです。これはリンクの受け渡しと安全な展開のための制限で、通常のWasm計算の入力・出力件数には追加制限を設けません。長いURLは共有先のアプリで途中までしか扱えない場合があります。上限を超える場合はCIDR一覧をコピーしてください。壊れたリンク、未知の形式、上限超過は画面にエラーを表示し、一部の共有内容だけを適用することはありません。

共有にはブラウザの`CompressionStream`・`DecompressionStream`のgzip対応が必要です。クリップボードへのコピーが拒否された場合は、選択してコピーできるリンク欄を表示します。「すべてリセット」は共有フラグメントもURLから削除します。変更した内容を再共有するときは、もう一度「共有リンクをコピー」を押してください。
