...
画面上部の「共有リンクをコピー」（英語版は`Copy share link`）で、その時点の状態を共有できます。初期集合、順序付きの追加・除外履歴、未適用の入力、IPv4・IPv6のズーム位置、選択範囲、一覧のページとタブを含みます。リンクを開くと同じGo Wasmで再計算し、画面を復元します。共有後の編集は互いに独立し、リアルタイム同期は行いません。
