...
ブラウザの計算はWeb Worker内のGo WebAssemblyで行います。静的ファイルの読み込み後は、通信を切っても編集・計算・ズームを使えます。入力をHTTP APIに送信しません。編集内容はメモリ内に保持します。通常のURLを再読み込みすると初期状態に戻り、共有リンクを再読み込みするとリンク作成時の内容に戻ります。
...
画面上部の「共有リンクをコピー」（英語版は`Copy share link`）で、その時点の状態を共有できます。初期集合、順序付きの追加・除外履歴、未適用の入力、IPv4・IPv6のズーム位置、選択範囲、一覧のページ位置を含みます。リンクを開くと同じGo Wasmで再計算し、画面を復元します。各パネルの開閉状態は共有しません。共有後の編集は互いに独立し、リアルタイム同期は行いません。

共有時は初期集合と入力欄の重複を差分で保存し、操作を短い配列で表現します。初期集合の順序・重複、追加／除外の順序、入力の空白・改行・未適用の編集をそのまま復元できます。最小CIDR一覧に置き換えて履歴を失うことはありません。

このJSONをgzipとBrotliで圧縮し、小さい方をbase64urlに変換して、URLの`#s=2.g.`または`#s=2.b.`以降に格納します。圧縮と復元は既存のWeb Worker内のGo Wasmで実行します。ブラウザのBrotli対応には依存せず、読み込み後はオフラインで共有リンクを作成できます。このフラグメントはHTTPリクエストに含まれず、計算API・DB・共有用サーバーは不要です。リンクを知っている人は入力途中の内容も復元できるため、共有前に内容を確認してください。暗号化やアクセス制限はありません。

共有リンクのフラグメントは32,768文字、差分を含む保存用JSONの展開後は8MiB、入力と履歴を復元した状態のJSONは16MiBまでです。URLの長さ超過とデータの展開・復元サイズ超過は、別のエラーメッセージで表示します。これらは共有に限った制限で、通常のWasm計算の入力・出力件数には追加制限を設けません。長いURLは共有先のアプリで途中までしか扱えない場合があります。壊れたリンク、未知の形式、上限超過では一部の共有内容だけを適用することはありません。

`2`は共有データ形式のバージョン、`g`／`b`は圧縮方式です。今後、保存する項目や意味を変えるときは形式のバージョンを上げ、公開済みの版のデコーダーと固定リンクのテストを残します。アプリの更新や圧縮率の調整だけで形式の版を変える必要はありません。今回は公開前の旧`#s=1.`形式への対応を終了しました。詳細は[共有形式の仕様](docs/share-format.md)を参照してください。

クリップボードへのコピーが拒否された場合は、選択してコピーできるリンク欄を表示します。「すべてリセット」は共有フラグメントもURLから削除します。変更した内容を再共有するときは、もう一度「共有リンクをコピー」を押してください。

連続範囲タブを選んで作成した旧v2リンクも開けます。その場合は連続範囲のパネルを開いて復元します。新しいリンクは同じv2形式のまま、従来のタブ項目を`cidrs`で保存します。
...
デプロイ直後は、検査中に固定URLの配信内容が切り替わることがあります。ETagと本文の両方が変わった場合だけ、対象ファイルの検査を最大4回やり直します（待機は1・3・10・20秒）。同じ版に対する不正な応答や、ハッシュ付きURLの内容変更はその場で失敗させます。

## 画面の使い方

初期集合の入力欄と最小CIDR一覧は常に表示します。「追加・除外」「集合の見取り図」「連続範囲」は初回は閉じており、見出しをクリックすると開閉できます。

1. 初期集合にIPまたはCIDRを1行ずつ入力し、「初期集合を適用」を押します。空行と行の前後の空白はUIが取り除きます。適用すると操作履歴を初期化します。
2. 「追加・除外」を開いて対象を指定し、「追加」「除外」を押します。操作履歴の上から順に集合演算を行います。初期集合に未適用の変更がある間は、追加・除外を無効にします。
3. 「集合の見取り図」を開き、IPv4とIPv6の帯で範囲を確認します。「集合に合わせる」「全体 /0」、IP・CIDR指定のズームを使えます。
4. 帯へのホバー・クリック・キーボード操作で開始IP、終了IP、関連CIDRを表示します。描画幅より小さな範囲はマーカーで示します。同じ位置に重なる範囲は、マーカーを選び左右矢印キーで切り替えます。「連続範囲」を開くと一覧から個別の範囲を選べます。
...
パネルの開閉状態はブラウザの`localStorage`（`cidr.panel-preferences.v1`）に保存し、再読み込み後も使います。日本語版と英語版で共通です。保存する値は`operations`、`visualization`、`ranges`の真偽値だけで、入力内容は保存しません。
...
internal/sharecodec/  上限付きgzip・Brotli圧縮と復元
...
tests/share/          共有形式の完全復元・ランダム検証
tests/fixtures/       将来も読み込みを維持する固定共有リンク
...
WorkerはWasmの関数登録完了を確認してから`ready`を返します。集合の計算と共有リンクの圧縮・復元を同じWorkerで処理します。要求と応答にはIDを付け、UIも編集世代を確認することで古い応答の反映を防ぎます。初期化失敗・計算エラーは画面に表示します。
