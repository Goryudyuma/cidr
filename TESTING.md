...
2026年9月24日、Go 1.27.1、Node.js 24.7.0、Playwright 1.62.1で確認しました。

- Wasm・フロントエンドの本番ビルド、TypeScript型検査、ESLint、Prettierが成功
- 既存29件と追加6件、計35件の実Chromiumテストを確認。初回の閉状態、操作件数の表示、キーボード操作、開閉状態の保存と日英切り替え、入力エラー時の自動展開、範囲選択による可視化の展開を検証
- 保存領域が使えない場合や設定が壊れている場合も、計算と開閉が動作。共有リンクは開閉設定を含まず、受け手の設定を維持。旧v2リンクの連続範囲タブも復元
- 大きな共有入力のテストで20万行の入力欄を消す自動操作が時間切れになったため、同テストの入力時と同じDOM経由の値設定・inputイベントに変更。共有内容の復元と、その後の追加操作が成功
- PC 1440px、日本語390px・英語320pxの折り畳み前後を目視確認。モバイルの横方向のはみ出しなし
- ローカル配信に公開確認スクリプトを実行し、共通37ケース、実Worker/Wasm、オフライン編集・言語切り替え・共有と復元が成功。ブラウザ実行時エラー0件
